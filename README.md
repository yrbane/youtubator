...
![version](https://img.shields.io/github/v/release/yrbane/youtubator?label=version&color=blue) — voir [CHANGELOG.md](CHANGELOG.md) ([English](CHANGELOG.en.md)). La version vit dans `apps/web/package.json`, est injectée au build (`__APP_VERSION__`) et affichée dans la topbar de l'app.
...
**Contribuer** : règles de contribution (sécurité des dépendances, testabilité des stores, frontière avec la config serveur, ADR, scan de secrets pre-commit, extraction `*-core.ts` de la logique à cycle de vie, `.gitignore` synchrone avec les outils de dev, `any` cantonné aux frontières API non typées, extensibilité des backends audio, stockage des tokens OAuth, filtrage `postMessage` cross-frame, typage des réponses JSON d'API HTTP externe, angle mort de `pnpm typecheck` sur `apps/web`/`extension`) dans [docs/GUIDELINES.md](docs/GUIDELINES.md).
