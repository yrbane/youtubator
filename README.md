...
**Contribuer** : règles de contribution (sécurité des dépendances, testabilité des stores, frontière avec la config serveur, ADR, scan de secrets pre-commit, extraction `*-core.ts` de la logique à cycle de vie, `.gitignore` synchrone avec les outils de dev) dans [docs/GUIDELINES.md](docs/GUIDELINES.md).
