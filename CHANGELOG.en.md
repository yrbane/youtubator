...
## 0.20.18 — 2026-09-02 · "The three permissions and nothing more"

- **`docs/GUIDELINES.md`**: rule 18 — `extension/manifest.src.json` only
  declares the Chrome permissions actually called in `extension/src`
  (`tabCapture`, `offscreen`, `downloads`), verifiable by grep: each
  permission has a matching `chrome.*` call, and the only other `chrome.*`
  calls in the code (`chrome.action`, `chrome.runtime`) are base MV3 APIs
  that don't need a declaration. Same minimalism for `content_scripts` and
  `web_accessible_resources`: only `youtube.com`/`youtube-nocookie.com`,
  never `<all_urls>`. Verified tonight — both greps confirm an exact
  perimeter, no orphan permission, no uncovered call.
- Dependency audit (`pnpm audit`) still at 19 dev-only alerts (5 moderate +
  14 high), identical to the ones already fixed on `lutin/ameliorations`
  (PR #2, `MERGEABLE`/`CLEAN`, CI green, open since 2026-08-14, still
  awaiting a merge decision — issues #10/#13): fixes already written on the
  branch, not replayed by hand on `main` to avoid duplicating them.
- `pnpm test` (296 tests), `pnpm typecheck` and `pnpm build` all green
  tonight; `gitleaks detect --source . --log-opts="--all"` (102 commits)
  found no leak.
...
- **`docs/GUIDELINES.md`**: rule 16 — the `ci`/`e2e` jobs in
  `.github/workflows/ci.yml` have installed with `pnpm install
  --frozen-lockfile=false` since the file's first commit, never commented
  on. This flag lets pnpm recompute the dependency resolution in memory if
  `package.json` drifts from `pnpm-lock.yaml`, instead of failing — the
  opposite of pnpm's strict default behavior outside CI. Verified tonight:
  `pnpm install --frozen-lockfile` (strict) succeeds with no changes at all
  on `main`, so this isn't a fix for anything currently broken, only a
  missing guard rail. The fix is blocked by the same limit as issue #20
  (the lutin's token lacks the `workflow` OAuth scope, so GitHub refuses
  any update to `.github/workflows/*.yml` through this channel): described
  in the guidelines for manual application, or automatic once the scope is
  granted.
- `pnpm test` (296 tests), `pnpm typecheck`, `pnpm build` and `gitleaks
  detect --source . --log-opts="--all"` (100 commits) all green tonight;
  `pnpm audit` still at 19 dev-only alerts already fixed on
  `lutin/ameliorations` (PR #2, `MERGEABLE`/`CLEAN`, CI green, still
  awaiting a merge decision — issues #10/#13).
